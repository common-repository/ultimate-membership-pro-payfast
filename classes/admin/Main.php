...
class Main
{
    ...
    public function addMagicFeatureItem( $items=[] )
    {
        $items[ $this->addOnSettings['slug'] ] = array(
                'label'						=> esc_html__( $this->addOnSettings['name'], 'ultimate-membership-pro-payfast' ),
                'link' 						=> (defined('IHCACTIVATEDMODE') && IHCACTIVATEDMODE) ? admin_url('admin.php?page=ihc_manage&tab=' . $this->addOnSettings['slug'] ) : '',
                'icon'						=>  'icon-ump-payfast-logo', //'fa-' . $this->addOnSettings['slug'] . '-ihc',
                'extra_class' 		=> 'ihc-extra-extension-box iump-' . $this->addOnSettings['slug'] . '-box',
                'description'			=> esc_html__('A Flexible Payment Solution to Process Payments for Any Type of Business', 'ultimate-membership-pro-payfast'),
                'enabled'					=> ihc_is_magic_feat_active( $this->addOnSettings['slug'] ),
        );
				if ( defined( 'UMP_PAYFAST_PRO' ) && UMP_PAYFAST_PRO ){
						// Pro Label if its case
						$items[ $this->addOnSettings['slug'] ]['label'] = esc_html__('Payfast Pro', 'ultimate-membership-pro-payfast');
				}
    ...
    public function styleAndScripts()
    {
        ...
        if ( $page && $page == 'ihc_manage' ){
            	wp_enqueue_style( $this->addOnSettings['slug'] . '-admin-style', $this->addOnSettings['dir_url'] . 'assets/css/admin.css' );
							wp_enqueue_style( $this->addOnSettings['slug'] . '-admin-font-style', $this->addOnSettings['dir_url'] . 'assets/css/fontello.css' );
		...
		public function box()
		{
				...
				$data['pay_stat'] = ihc_check_payment_status( 'ump_payfast' );
		...
		public function paymentSettings( $tab='' )
		{
				...
				do_action( 'ump_addon_action_before_print_admin_settings' );
				$ihcSave = isset( $_POST['ihc_save'] ) ? sanitize_text_field( $_POST['ihc_save'] ) : false;
				if ( $ihcSave ){
						ihc_save_update_metas( $this->addOnSettings['slug'] );//save update metas
				}
				$data = ihc_return_meta_arr( $this->addOnSettings['slug'] );
